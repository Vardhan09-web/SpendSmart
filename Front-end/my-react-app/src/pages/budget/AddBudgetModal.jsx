...
export default function AddBudgetModal({ onClose, isDark, refreshBudgets }) {
  const [open, setOpen] = useState(false);
  const [category, setCategory] = useState("");
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 px-4">
      <div className={`w-full max-w-md rounded-2xl p-6 ${isDark ? "bg-[#0f1a2e]" : "bg-white"}`}>
        ...
        <div className="flex gap-4 mt-6">
          ...
          <button
            onClick={handleSave}
            className="flex-1 py-3 rounded-xl bg-gradient-to-r from-emerald-500 to-blue-500 font-semibold text-white"
          >
            Save
          </button>
        </div>
